...
public class Progetto implements Named{
	...
	/**
	 * Crea un nuovo it.unicam.cs.DOIT.business.Progetto
	...
	public String toString() {
		...
		for(Utente u: Bacheca.getCatalogoUtenti())//Cerco il nome del creatore nel catalogo utenti
	...
	public Collection getCandidati() {
		...
		for (Partecipazione p : partecipazioni)
			if(p.getStato()==StatiPartecipazione.NON_CONFERMATO)
	 ...
	 * @return partecipanti
	 */
	public Collection<String> getPartecipanti(){
		Collection<String> names=new HashSet<>();
		for (Partecipazione p : partecipazioni)
			if(p.getStato()==StatiPartecipazione.CONFERMATO)
				names.add(p.getProgettista().getID()+"> "+p.getProgettista().getNome() + " " +p.getProgettista().getCognome());
		return names;
	}
	/**
		...
		if(getSpecifiche().length()<50)
			puntini="";
				String retn=getTitolo()+"\n"+
						...
						getSpecifiche().substring(0,50)+puntini;
		return getTitolo();
	}
